...
def longest_substring_with_k_distinct(str, k):
    ...
    for window_end in range(len(str)):
        b_pointer = str[window_end]
        if b_pointer not in char_frequency:
            char_frequency[b_pointer] = 0
        char_frequency[b_pointer]+=1

    # shrink from left
        while len(char_frequency) > k:
            a_pointer = str[window_start]
            char_frequency[a_pointer] -= 1
            if char_frequency[a_pointer]==0:
                del char_frequency[a_pointer]
            window_start+=1 # shrink the window from left
